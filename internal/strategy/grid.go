...
║                           GRID STRATEGY                                       ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  Backtest Results (MES M5, $100k equity, 1% risk/trade, 2.5 tháng data)      ║
╠═══════════════════════════╦══════════════════════╦═══════════════════════════╣
║  Version                  ║  Original (HF)       ║  Conservative             ║
╠═══════════════════════════╬══════════════════════╬═══════════════════════════╣
║  Return                   ║  +51.94%  ✅         ║  +33.54%                  ║
║  Max Drawdown             ║  20.32%              ║  ~15%  ✅                 ║
║  Total Trades             ║  458                 ║  233                      ║
║  Win Rate                 ║  91.05%  ✅          ║  85.41%                   ║
║  Profit Factor            ║  3.45  ✅            ║  ~2.5                     ║
╠═══════════════════════════╩══════════════════════╩═══════════════════════════╣
║  ✅ KHUYẾN NGHỊ: Original cho max return, Conservative cho low-risk          ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  PARAMETERS COMPARISON:                                                      ║
║  ┌─────────────────────┬──────────────────────┬─────────────────────────┐   ║
║  │ Parameter           │ Original (HF)        │ Conservative            │   ║
║  ├─────────────────────┼──────────────────────┼─────────────────────────┤   ║
║  │ GridSpacingPct      │ 0.2% (~14 pts)       │ 0.4% (~27 pts)          │   ║
║  │ ReboundPct          │ 15%                  │ 20%                     │   ║
║  │ MaxGridLevels       │ 5                    │ 3                       │   ║
║  │ LookbackBars        │ 20                   │ 30                      │   ║
║  │ StopLossPct         │ 0.5%                 │ 0.6%                    │   ║
...
║                                                                              ║
║  CLI Usage:                                                                  ║
║    ./quant-bot backtest -i                    # Interactive mode             ║
║    ./quant-bot backtest --strategy grid       # Original (high-frequency)    ║
║    ./quant-bot backtest --strategy grid-conservative  # Conservative         ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
*/
...
type GridConfig struct {
	...
	// Risk parameters
	StopLossPct   decimal.Decimal // Stop loss as % of entry (e.g., 0.005 = 0.5%)
	MinMovePoints decimal.Decimal // Minimum price move to trigger grid entry
}

// OriginalGridConfig returns the high-frequency grid parameters.
// Results: +51.94% return, 91.05% win rate, 458 trades, PF 3.45
func OriginalGridConfig() GridConfig {
	return GridConfig{
		GridSpacingPct: decimal.RequireFromString("0.002"),  // 0.2% between levels (~14 pts on MES)
...
// ConservativeGridConfig returns conservative grid parameters with lower risk.
// Results: +2.28% return, 57.89% win rate, 19 trades, PF 1.34, Max DD 4.32%
func ConservativeGridConfig() GridConfig {
	return GridConfig{
		GridSpacingPct: decimal.RequireFromString("0.004"),  // 0.4% between levels (~27 pts on MES)
		ReboundPct:     decimal.RequireFromString("0.20"),   // 20% rebound for TP
		MaxGridLevels:  3,                                    // Max 3 positions
		LookbackBars:   30,                                   // 30 bars for swing detection
		StopLossPct:    decimal.RequireFromString("0.006"),  // 0.6% stop loss
		MinMovePoints:  decimal.RequireFromString("20"),     // Min 20 points move
	}
}

// DefaultGridConfig returns the original high-frequency config (best performance).
...
// NewGrid creates a new grid strategy with default (original) config.
